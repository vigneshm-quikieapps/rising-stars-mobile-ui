    ...
    return (
        <View style={[styles.subcontainer,props.substyle]}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                <Text style={{fontSize:hp('2%'),fontFamily:"Nunito-SemiBold",}}>£{props.amount}</Text>
                <View style={[styles.notify,props.style]}>
                    <Text style={{color:colors.white,fontFamily:"Nunito-SemiBold",}}>{props.notify}</Text>
                </View>
            </View>
            <Text style={{fontFamily:"Nunito-SemiBold",}}>{props.body}</Text>
            <Text style={{fontFamily:"Nunito-SemiBold",}}>{props.date}</Text>
            {
                props.button ?<AppButton title={props.title} style={props.paystyle} onPress={props.paybutton}/> :null
            }
            {
                props.paid ? <Text style={{fontFamily:"Nunito-SemiBold",}}>{props.paidtext}</Text>: null
            }
        </View>
    )
}
